...
MCC_fnc_VONRadioBroadcast = 
{
	...
	switch _channelIndex do 
	{
		...
		case 1 : //Side
		{
			...
			_allowed	= if (player == leader player) then {true} else {false}; 

		};	
		case 2 : //Commander
		{
			_radioRange = missionNameSpace getVariable ["MCC_vonRadioDistanceCommander",30000]; 
			_allowed	= if (getPlayerUID  player == (MCC_server getVariable [format ["CP_commander%1",(player getVariable ["CP_side",  playerside])],""])) then {true} else {false}; 
		};	
		case 3 : //Group
		{
