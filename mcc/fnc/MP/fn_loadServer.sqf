...

{
	...
	if (_loadTickets) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "TICKETS", str _x, "read",100, true] call MCC_fnc_handleDB;
	...
	if (_loadResources) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "RESOURCES", str _x, "read",(missionNameSpace getVariable [format ["MCC_res%1",_x],[1500,500,200,200,200]]),true] call MCC_fnc_handleDB;
	...
	if (_loadCivRelation) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "CIV_RELATION", str _x, "read",(missionNamespace getvariable [format ["MCC_civRelations_%1",_x],0.5]),true] call MCC_fnc_handleDB;
	...
	//Load CAS
	if (_loadCAS) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "CAS", str _x, "read",(missionNamespace getvariable [format ["MCC_CASConsoleArray%1",_x],[]]),true] call MCC_fnc_handleDB;
	...
	if (_loadAirDrop) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "AIR_DROP", str _x, "read",(missionNamespace getvariable [format ["MCC_ConsoleAirdropArray%1",_x],[]]),true] call MCC_fnc_handleDB;
	...
	//Load artillery
	if (_loadTickets) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "ARTILLERY", str _x, "read",(missionNamespace getvariable [format ["Arti_%1_shellsleft",_x],0]),true] call MCC_fnc_handleDB;
	...
	if (_loadStartLoc) then {
		_tempVar = [format ["%1_%2",_fileName, worldname], "SPAWN_POINTS", str _x, "read",[],true] call MCC_fnc_handleDB;
...
if (_loadRTSBuildngs) then {
	_tempVar = [format ["%1_%2",_fileName, worldname], "BUILDINGS", "BUILDINGS", "read",[],true] call MCC_fnc_handleDB;
	{_x spawn MCC_fnc_construct_base} forEach _tempVar;
};

//Load Arty types
if (_loadArtilleryTypes) then {
	_tempVar = [format ["%1_%2",_fileName, worldname], "ARTILLERY", "TYPE", "read",(missionNamespace getvariable ["HW_arti_types",[["HE Laser-guided","Bo_GBU12_LGB",3,50],["HE 82mm","Sh_82mm_AMOS",1,75]]]),true] call MCC_fnc_handleDB;
