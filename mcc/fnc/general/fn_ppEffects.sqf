...
switch (tolower _effect) do
{
	...
	{ 
		...
		if !(_jip) then
		{
			setWind [10, 10, true];
			0 setWindStr 1;
			[[[ 0.5, 1, 1, 0, 1,0.7]],"MCC_fnc_setWeather",true,false] spawn BIS_fnc_MP;
		};
	};
	...
	{ 
		...
		if !(_jip) then
		{
			setWind [10, 10, true];
			0 setWindStr 1;
			[[[ 1, 1, 1, 0.4, 1,0.7]],"MCC_fnc_setWeather",true,false] spawn BIS_fnc_MP;
		};
		player setVariable ["MCC_ppEffectRuning",_effect];
	};
	...
	{ 
		...
		if !(_jip) then
		{
			setWind [10, 10, true];
			0 setWindStr 0;
			[[[ 0, 0, 0, 0, 0,0]],"MCC_fnc_setWeather",true,false] spawn BIS_fnc_MP;
		};
