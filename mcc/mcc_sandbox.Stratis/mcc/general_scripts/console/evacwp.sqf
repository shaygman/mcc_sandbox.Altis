...
private ["_type","_flyInHight","_landing","_point1","_convoy_wp1","_convoy_wp2","_point2","_convoy_wp3","_point3","_evac"];
_type = _this select 0; 
if (count MCC_evacVehicles == 0) exitwith {hint "No Evac is under your command"};
_evac = MCC_evacVehicles select (lbCurSel MCC_ConsoleEvacTypeText_IDD);
...
if (_type == 0) then {														// 1 WP
	//hint "Markers placed";
	[[[_point1], _flyInHight, _landing, netid _evac], "MCC_fnc_evacMove", true, false] call BIS_fnc_MP;
	} else {																// 3 WP
		...
		hint "Markers placed";
		[[[_point1, _point2, _point3], _flyInHight, _landing, netid _evac], "MCC_fnc_evacMove", true, false] call BIS_fnc_MP;
		};
