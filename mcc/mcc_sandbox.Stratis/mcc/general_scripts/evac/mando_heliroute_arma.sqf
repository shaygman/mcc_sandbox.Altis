...
if (_height == 5000) then {								//We got a vehicle
	...
	if (_landing == 3 || _landing == 4) then {
			 ...
			 while {(((getpos _heli) select 2)> ((_endpos select 2) + 1))&& (alive _pilot) && (damage _heli < 0.5)} do
			   {
				...
				[[netid _heli, "Golf 1 touchdown",false], "MCC_fnc_groupchat", true, false] spawn BIS_fnc_MP;
			...
			if (_landing == 6) then	{
				...
				[[netid _heli, "Golf 1 in position, clear for helocasting",false], "MCC_fnc_groupchat", true, false] spawn BIS_fnc_MP;
