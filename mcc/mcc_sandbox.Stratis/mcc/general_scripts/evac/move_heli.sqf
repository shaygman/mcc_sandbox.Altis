...
if !mcc_isloading then	{
	...
	if (MCC_capture_state) then
		...
		MCC_capture_var = MCC_capture_var + FORMAT ['
							[ [%1, %2, %3, netid %4],"MCC_fnc_evacMove",true,false] spawn BIS_fnc_MP;
							'
			...
			{
			[[[_point1, _point2, _point3], _flyInHight, _landing,netid _evac],"MCC_fnc_evacMove",true,false] spawn BIS_fnc_MP;
			};
};
