		...
		if (_isEngineer==1) then	//If it is a bomb expert ;)
						{
							if (_rand > 0.30) then {
													...
													if (_rand >0.05) then {
																		hint "Fail to disarm";
																		if (_randsound > 5) then {[[netid _men, "disarmfail1"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP} else {[[netid _men, "disarmfail2"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP};
																		}
																	else {
																		hint "Critical fail start runing";
																		_ied removeAction _index;
																		if (_randsound > 5) then {[[netid _men, "disarmcrit1"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP} else {[[netid _men, "disarmcrit2"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP};
																		_ied setvariable ["armed",false,true];
																		sleep 2 + random 3;
							...
							if (_rand > 0.70) then {	//If it isn't a bomb expert <*Kaboom*>
													hint "disarmed";
													if (_randsound > 5) then {[[netid _men, "disarm3"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP} else {[[netid _men, "disarm4"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP};
													sleep 1;
													_ied setvariable ["armed",false,true];
													_ied removeAction _index;
													_dummyMarker = _ied getvariable "iedMarkerName"; 
												...
												hint "Fail to disarm";
													if (_rand >0.3) then {
																		if (_randsound > 7) then {[[netid _men, "disarmfail1"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP} else {[[netid _men, "disarmfail2"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP};
																	} else {
																		hint "Critical fail start runing";
																		_ied removeAction _index;
																		if (_randsound > 5) then {[[netid _men, "disarmcrit1"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP; player switchmove "";} else {[[netid _men, "disarmcrit2"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP};
																		_ied setvariable ["armed",false,true];
																		sleep 3 + random 3;
																		"SmallSecondary" createVehicle _pos;
																		sleep 3 + random 3;
