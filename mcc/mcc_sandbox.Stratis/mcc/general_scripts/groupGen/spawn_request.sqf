...
if !mcc_isloading then	{
	...
	if (_type==1) then	{											//Add WP
		if (count MCC_groupGenGroupArray > 0) then {				//Fali safe if no group selected
			_wpType = ["MOVE", "DESTROY", "GETIN", "SAD", "JOIN", "LEADER", "GETOUT", "CYCLE", "LOAD", "UNLOAD", "TR UNLOAD", "HOLD", "SENTRY"
			   ,"GUARD","TALK","SCRIPTED","SUPPORT","GETIN NEAREST","DISMISS","AND","OR"] select (lbCurSel MCC_GroupGenWPType_IDD);
			...
			hint "click on the map"; 
				onMapSingleClick " 	hint ""WP Added."";
									[[0,_pos, MCC_WpGenarray, netID MCC_groupGenGroupWP],'MCC_fnc_manageWp',true,false] call BIS_fnc_MP;
	...
	if (_type==2) then	{										//ClearWP
		if (count MCC_groupGenGroupArray > 0) then {
			...
			hint "WP Cleared";
			[[1,[],[],netID MCC_groupGenGroupWP],'MCC_fnc_manageWp',true,false] call BIS_fnc_MP;
			sleep 1; 
			_nul=[1,MCC_groupGenGroupStatus] execVM MCC_path + "mcc\general_scripts\groupGen\group_manage.sqf";	//Refresh the group WP
			};
		};
