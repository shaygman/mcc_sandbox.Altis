...
//start the drop
if (_planeType=="C130J_US_EP1") then 		//If it's an airdrop
{
	//Lets Spawn a plane
	_plane1 			= [_planeType, _spawn, _pos, 100, false] call MCC_CREATE_PLANE;		//Spawn plane 1 
	...
	for [{_x=0},{_x < count _spawnkind},{_x=_x+1}] do 
		...
		[_planepos, _spawnkind select _x, _pilot1] spawn MCC_CreateAmmoDrop;
		sleep 0.5 + random 0.5;
		};
	//Delete the plane when finished
	[_pilotGroup1, _pilot1, _plane1, _away] call MCC_DELETE_PLANE;
}	
else 
{	
...
diag_log format ["gun-run: [%1]", _this];
		...
		//Lets Spawn a plane
		_plane1 			= [_planeType, _spawn, _pos, _planeAltitude, false] call MCC_CREATE_PLANE;		//Spawn plane 1 
		_pilotGroup1		= group _plane1;
		_pilot1				= driver _plane1;
		...
		{
			sleep 5;
			[_pilotGroup1, _pilot1, _plane1, _away, 150] spawn MCC_DELETE_PLANE;
		}
		else // chopper
		{
			sleep 3; // otherwise chopper will halt, climb, and continue
			[_pilotGroup1, _pilot1, _plane1, _away, 100] spawn MCC_DELETE_PLANE;
		};
	...
	else		
	{
		...
		_plane2 			= [_planeType, _spawn, _pos, 100, false] call MCC_CREATE_PLANE;		//Spawn plane 2 
		...
		switch (_spawnkind) do 
		{
		   ...
		   case "LGB":    //LGB 
				...
				for [{_x=1},{_x<=_ammount},{_x=_x+1}] do 
					{
						...
						[[netid _bomb, format["bon_Shell_In_v0%1",[1,2,3,4,5,6,7] select round random 6]], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
						sleep 0.25;
						...
						[[netid _bomb2, format["bon_Shell_In_v0%1",[1,2,3,4,5,6,7] select round random 6]], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
						sleep 0.25;
					};
			};
			...
			{ 
				...
				for [{_x=1},{_x<=_ammount},{_x=_x+1}] do 
					{
						_nul=[[(_pos select 0)+20 - random 40,(_pos select 1)+20 - random 40,_pos select 2], getpos _plane1,"M_AT",200,true,""] execVM MCC_path + "mcc\general_scripts\CAS\missile_guide.sqf";
						[[netid _plane1, "missileLunch"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
						sleep 0.6;
						_nul=[[(_pos select 0)+30 - random 100,(_pos select 1)+30 - random 60,_pos select 2], getpos _plane1,"M_AT",200,true,""] execVM MCC_path + "mcc\general_scripts\CAS\missile_guide.sqf";
						[[netid _plane1, "missileLunch"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
						sleep 0.6;
					};
			};
			...
			{ 
				...
				for [{_x=1},{_x<=_ammount},{_x=_x+1}] do 
					{
						_planepos = getpos _plane1;
						[_planepos, _pilot1] spawn MCC_SADARM;
						sleep 0.5 + random 0.5;
					};
			};
			
			case "CBU-Mines":	//MCC_CBU-Mines
			{ 
				...
				for [{_x=1},{_x<=_ammount},{_x=_x+1}] do 
					{
						...
						[_bomb, CBU_type] spawn MCC_CBU;
						sleep 0.5 + random 0.5;
					};
			};
			
			case "CBU-WP(ACE)":	//MCC_CBU-Mines
			{ 
				CBU_type = MCC_CBU_WP;
				waitUntil {((_plane1 distance [_pos select 0, _pos select 1, 200]) < 200) || (!alive _plane1)};
				//Make the drop
		...
		[_pilotGroup1, _pilot1, _plane1, _away] call MCC_DELETE_PLANE;
		[_pilotGroup2, _pilot2, _plane2, _away] call MCC_DELETE_PLANE;
	};
};
