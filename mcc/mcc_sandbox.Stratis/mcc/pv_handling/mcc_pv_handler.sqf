		...
		private ["_p_mcc_player","_p_mcc_player_name","_p_mcc_request","_mccdialog"];
				...
				switch (mcc_missionmaker) do {			//MM is logging out
					case _p_mcc_player_name:
						{
							[[netId _p_mcc_player, format["MCC ID %1-> %2 Logged out as Misson Maker.",_p_mcc_request,mcc_missionMaker], false],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
							mcc_missionmaker="";
					...
					case "":							//MM is logging out
						{
							...
							[[netId _p_mcc_player, format["MCC ID %1-> Access granted to: %2",_p_mcc_request,mcc_missionMaker], false],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
							publicVariable "mcc_missionmaker";
							ctrlSetText [MCCMISSIONMAKERNAME, format["%1",mcc_missionmaker]];
						};
				};
	...
	//------------ End Special Zone Stuff ----------
		...
		if (mcc_missionmaker == _p_mcc_player_name) then 		
				...
				{	
					...
					if (format["%1",_safepos] != "[-500,-500,0]" ) then
					{
						...
						if (_p_mcc_spawntype == "MAN") then
							{
								...
								[[netId _p_mcc_player, format["MCC ID %1-> Spawned ""%2"" of type %3.",_p_mcc_request,_unitspawned,_p_mcc_spawnname], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
								...
								if (_p_mcc_spawnwithcrew and (_p_mcc_spawntype == "VEHICLE") ) then
									{
										...
										switch (_p_mcc_classtype) do
											{
												case "AIR":
												{
													if (_p_mcc_zone_behavior != "bis") then {
										...
										[[netId _p_mcc_player, format["MCC ID %1-> Spawned ""%3"" of type %2.",_p_mcc_request,_p_mcc_spawnname,(_unitspawned select 0)], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
									}
								else
									{
										//Vehicle without any crew, so here we go 
										_unitspawned 	= _p_mcc_spawnname createVehicle _safepos;
										[[netId _p_mcc_player, format["MCC ID %1-> Spawned type %2.",_p_mcc_request,_p_mcc_spawnname], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
									};
						...
						if (_p_mcc_spawntype == "GROUP") then
							
							{
										...
										[[netId _p_mcc_player, format["MCC ID %1-> Spawned ""%3"" of type %2.",_p_mcc_request,_p_mcc_spawnname,((units _unitspawned) select 0)], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
					...
					if _p_mcc_iszone_update then
						{
							[[netId _p_mcc_player, format["MCC ID %1-> Created/Updated zone: %2.",_p_mcc_request,_p_mcc_zone_markername], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
						}
					else
						{
							...
							then
								{
									[[netId _p_mcc_player, format["MCC ID %1-> Spawned in grid: %2.",_p_mcc_request,(_safepos call BIS_fnc_PosToGrid)], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
								}
							else
								{
									[[netId _p_mcc_player, format["MCC ID %1-> SPAWN FAILED! No good position found!",_p_mcc_request], true],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
								};
						};
					};
				}
				...
				{
					[[netId _p_mcc_player, format["MCC ID %1->: Server access denied for: %2.",_p_mcc_request,_p_mcc_player], false],"MCC_fnc_groupchat",true,false] spawn BIS_fnc_MP; 
				};
	};
