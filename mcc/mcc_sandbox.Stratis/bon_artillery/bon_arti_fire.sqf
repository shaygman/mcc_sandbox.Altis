...
{//foreach _cannons_to_fire
	...
	if(_available) then{
		...
		[_x,_cannonsetup,_requestor,BON_salvo] spawn {
			...
			[[netid (MCC_bonCannons select (_cannon-1)), 0],"MCC_fnc_broadcast",true,false] spawn BIS_fnc_MP;
			...
			if (MCC_bonFire) then {
				MCC_bonFire = false;
				[[netid _requestor, "shoutS5"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
				sleep 2; 
				//[playerSide,"HQ"] sideChat "Shot, Over";
				[[netid _requestor, "shoutO5"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
				};
			...
			if (MCC_bonSplash) then {
				MCC_bonSplash = false;
				[[netid _requestor, "splashS6"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
				sleep 2; 
				//[playerSide,"HQ"] sideChat "Shot, Over";
				[[netid _requestor, "splashO6"], "MCC_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
				};
				...
				switch (_artitype) do
					{
						...
						{ 
							[_splashpos, _artitype, _spread, 1, true] spawn MCC_ARTY_DPICM;
						};
						case "Smoke_120mm_AMOS_White":	//Smokeshell 
						{ 
							[_splashpos, _artitype, _spread, 1, true] spawn MCC_ARTY_FLARE;
						};
						...
						default	//bomb
						{ 
							[_splashpos, _artitype, _spread, 1, true] spawn MCC_ARTY_BOMB;
						};
					};
