...
if (isnil "MCC_CONST_CAM") then {
	...
	_camera camConstuctionSetParams [_startPos,-1,300];
...
MCC_fnc_rtsCreateUICtrl = {
	private ["_ctrlColor","_display","_ctrl","_idc","_group","_texture"];
	disableSerialization;
	...
	_idc = (uiNamespace getVariable ["MCC_idcCounter",1212])+1;
	uiNamespace setVariable ["MCC_idcCounter",_idc];
		...
		if (_key == 0) exitWith	{
			...
			if (count _list > 0 && !_isFort) then {
				...
				if (side _obj in [playerSide,sideLogic,civilian,sideFriendly,sideUnknown]
				    ...
				    &&
				    ((count crew _obj == 0) || _obj isKindOf "logic")
				    ) then {
		...
		if (!isnil "MCC_mousePos") then {
			...
			if (!isnull MCC_CONST_PLACEHOLDER) then {
				...
				_isFlat = _pos isflatempty [
					...
					20,				//--- Max gradient
					(sizeof typeof MCC_CONST_PLACEHOLDER)/4,	//--- Gradient area
