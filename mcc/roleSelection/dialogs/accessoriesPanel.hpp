class CP_ACCESPANEL {
	  idd = -1;
	  movingEnable = false;
	  onLoad =  __EVAL("_this execVM '"+CPPATH+"mcc\roleSelection\scripts\accessPanel_init.sqf'");
	  ...
	  controls[] =
	  {
		...
		class CP_uniformPanelBackButton: CP_RscButtonMenu
		{
			...
			action = __EVAL("[4] execVM '"+CPPATH+"mcc\roleSelection\scripts\switchDialog.sqf'");
		};
		...
		class CP_accessoriesPanelOptics: CP_RscCombo
		{
			...
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 1)";
			onLBSelChanged = __EVAL("[0] execVM '"+CPPATH+"mcc\roleSelection\scripts\accessPanel_cmd.sqf'");
		};
		class CP_accessoriesPanelBarrel: CP_RscCombo
		{
			...
			h = 0.0879658 * safezoneH;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 1)";
			onLBSelChanged = __EVAL("[1] execVM '"+CPPATH+"mcc\roleSelection\scripts\accessPanel_cmd.sqf'");
		};
		class CP_accessoriesPanelAttachment: CP_RscCombo
		{
			...
			h = 0.0879658 * safezoneH;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 1)";
			onLBSelChanged = __EVAL("[2] execVM '"+CPPATH+"mcc\roleSelection\scripts\accessPanel_cmd.sqf'");
		};
