...
for [{_x = 1},{_x <=3},{_x = _x+1}] do		
{
	...
	if (_objType != "None") then 
	{
		...
		if (_isCQB) then 
		{
			[[_objPos,(_maxObjectivesDistance*0.5),0,(_totalEnemyUnits*0.005),_faction,str _side],"MCC_fnc_garrison",false,false] spawn BIS_fnc_MP;
		}; 
		
		// Is _isCiv
		if (_isCiv) then 
		{
			[[_objPos,(_maxObjectivesDistance*0.5),1,(_totalEnemyUnits*0.005),"CIV_F","CIV"],"MCC_fnc_garrison",false,false] spawn BIS_fnc_MP;
		};
