...
_willseearray = [_unit,_firer] call gaia_fnc_willseetarget;
...
if (_willseearray select 0) then 
{
	...
	if (! (isNull _currentTarget)) then
	{
		...
		if (_newDist <= (_currentDist + 1)) then
		{
			...
			[_unit,_firer] call gaia_fnc_cqc_target;
...
else 
{
	...
	if (!_seeking) then
	{
		...
		if ((count _buildUnits) >= 4) then
		{
			...
			[_searchTeam,_alertPos,_build] spawn gaia_fnc_seek;
		};
	};
	*/
	...
	_angle = ([_unit,_firer] call gaia_fnc_get_angle);
	_willsee = false;
	_target = [];
	_targetATL = [];
	_acnt = 0;
	...
	{	
		...
		_willseearray = [_unit,_angle,_dist] call gaia_fnc_willseeincombat;
	...
	[_unit,_targetATL] call gaia_fnc_cqc_target;
