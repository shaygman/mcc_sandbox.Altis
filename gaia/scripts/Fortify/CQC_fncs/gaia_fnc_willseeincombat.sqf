...
_bDoor = false;
	...
	if (!(_willsee)) then
	{
		...
		_doorPositions = _obstruction getVariable ["doorPositions",nil]; if (isNil ("_doorPositions")) then {_doorPositions = [_obstruction] call gaia_fnc_get_DoorPositions};
		_intersectWorldPos = [_unit,_infront] call gaia_fnc_Intersect_Pos_Rough;
