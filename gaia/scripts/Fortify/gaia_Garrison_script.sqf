...
{
	alertdebug = [] execVM (GAIA_scripts + "Fortify\Garrison_fncs\alerted_debug.sqf";
};
*/
...
{	
	...
	{
		nul = [_x] spawn gaia_fnc_vision_check;
		nul = [_x] spawn CQC_AI;
	};
} foreach allunits;
...
// building randomly selected and array of positions generated
	...
	if ((count _buildingsleft) >= 1) then 
	{
		...
		_localbuilding setvariable ["doorPositions",([_localbuilding] call gaia_fnc_get_DoorPositions)]; 
		...
		_sillyarray = [_localbuilding] call gaia_fnc_sillybuild_check;
			...
			if ((count _bposleft) >= 1) then 
			{
				...
				nul = [_x,_poscoords,_patrolgroup,_stationary,_warping] spawn {
					...
					{	
						...
						_moveComplete = [_unit,_dpos] call gaia_fnc_MoveTo;
					};
...
if ((count units _patrolgroup) > 0) then 
{
	...
	nul = [leader _patrolgroup,_patrolCenter,_patrolRadius,true] spawn gaia_fnc_Patrol;
	...
	while {sleep 1;true} do 
	{
		...
		if ((count units _patrolgroup) > 4) then 
		{
			...
			nul = [leader _newgroup,_patrolCenter,_patrolRadius,true] spawn gaia_fnc_Patrol;
		};
	};
};
