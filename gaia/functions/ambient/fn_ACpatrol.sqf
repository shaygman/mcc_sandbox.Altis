...
while{true}do{
	...
	while{_i < (count LV_ACS_activeGroups)}do{
		...
		if(((typeName _sUnit) == "ARRAY")||(_mp))then{
			if(_mp)then{ _sUnit = call GAIA_fnc_GetPlayers;};
		...
		if((count (waypoints _wGroup ) == (currentWaypoint _wGroup)))then{
			...
			while{!_validSpot}do{
				...
				if(_validSpot)then{
					...
					{
						_area = _x;
						if([_newPos,_area] call GAIA_fnc_IsInMarker)exitWith{_validSpot = false;};
						...
						while{_dis > 0}do{
							...
							if([_tPos,_area] call GAIA_fnc_IsInMarker)exitWith{_validSpot = false;};
							_dis = _dis - 15;
						};
